...
async def test_trending_increment_failure_does_not_poison_outer_transaction(
    ...
    failed_once = False
    dialect_name = async_session.get_bind().dialect.name
    ...
    async def flaky_flush(*args, **kwargs):
        ...
        if not failed_once:
            failed_once = True
            raise IntegrityError("flush trending_keywords", {}, Exception("duplicate entry"))
...
@pytest.mark.asyncio
async def test_trending_increment_keeps_stale_keyword_count(async_session: AsyncSession):
    """마지막 검색 시각이 오래됐어도 search_count는 계속 누적 증가합니다."""
    stale_keyword = TrendingKeyword(
        keyword="아이언맨",
        search_count=7,
        last_searched_at=datetime.now(timezone.utc) - timedelta(days=4),
    )
    async_session.add(stale_keyword)
    await async_session.commit()

    repo = TrendingRepository(async_session)
    updated = await repo.increment("아이언맨")
    await async_session.commit()

    assert updated.search_count == 8
